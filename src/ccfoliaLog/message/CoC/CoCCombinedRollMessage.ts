...
export class CoCCombinedRollMessage extends CcfoliaMessage {
    skill: string;
    diceValue: number;
    successValue: [number, number];
    isSecret: boolean;

    constructor(data: CcfoliaMessageData, skill: string, diceValue: number, successValue: [number, number], isSecret: boolean = false) {
        super(data);
        this.skill = skill;
